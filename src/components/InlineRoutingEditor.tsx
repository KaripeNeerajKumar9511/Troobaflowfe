...
interface InlineRoutingEditorProps {
  ...
  onAddRoute: (toOpName: string, pct: number) => void | Promise<void>;
...
export function InlineRoutingEditor({
  ...
  const wrap = (
    ...
    child: ReactElement,
    getValue?: () => unknown,
  ) => (wrapRouteCell ? wrapRouteCell(routeId, field, child, getValue) : child);
  ...
  const handleAdd = () => {
    if (!newTo) return;
    void Promise.resolve(onAddRoute(newTo, newPct)).then(() => {
      setNewTo('');
      setNewPct(100);
    });
  };
            ...
            {routes.map(r => {
              ...
              <div key={r.id} className="flex items-center gap-2">
                ...
                {wrap(
                  ...
                  () => routes.find((x) => x.id === r.id)?.to_op_name,
                )}
