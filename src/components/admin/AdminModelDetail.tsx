...
export function AdminModelDetail() {
  const { userId, modelId } = useParams<{ userId: string; modelId: string }>();
  ...
  useEffect(() => {
    if (payload && userId) {
      ...
      setTitleLink(`/TF-admin/users/${userId}`);
    }
    return () => setTitleLink(null);
  }, [payload, userId, setContextTitle, setContextSubtitle, setTitleLink]);
  ...
  return (
    <div className="p-6 max-w-6xl animate-fade-in">
      <div className="mb-6">
        <button
          type="button"
          onClick={() => navigate(`/TF-admin/users/${userId}`)}
