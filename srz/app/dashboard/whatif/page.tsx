...
export default function WhatIfStudio() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast.success(`What-if "${name}" created and active.`);
    } catch (err) {
      console.error('Create what-if failed:', err);
      toast.error('Failed to create What-if. Please try again.');
    } finally {
      setIsCreating(false);
    }
  };
  ...
  return (
    <div className="flex flex-row min-h-[calc(100vh-8rem)] w-full overflow-hidden bg-white rounded-lg border border-slate-200/80 shadow-sm">
      {/* Left: Scenario list panel — 240px, light grey, thin divider to content */}
      <div className="w-[240px] min-w-[240px] shrink-0 flex flex-col overflow-hidden bg-[#FAFAFA] border-r border-slate-200">
          
          <div className="h-9 flex items-center border-b border-slate-200 shrink-0 px-2">
            <button
            ...
            >
              Scenarios
              {leftTab === 'scenarios' && <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#20B2AA] rounded-full" />}
            </button>
            {showFamilies && (
              <button
              ...
              >
                Families
                {leftTab === 'families' && <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#20B2AA] rounded-full" />}
              </button>
            )}
          </div>
            ...
            <>
              <div className="flex-1 overflow-y-auto">
                ...
                >
                  ...
                  <span className="text-sm font-medium text-slate-800 flex-1 truncate">Basecase</span>
                  {!activeScenarioId && (
                    <span className="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium bg-[#4CAF50]/20 text-[#2E7D32]">Active</span>
                  )}
                ...
                {scenarios.map((sc, idx) => {
                  ...
                  if (isActive) {
                    statusBadge = <span className="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium bg-[#4CAF50]/20 text-[#2E7D32]">Active</span>;
                    ...
                    statusBadge = <span className="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium bg-[#4CAF50]/20 text-[#2E7D32]">Results Current</span>;
                  } else {
                    statusBadge = <span className="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium bg-slate-100 text-slate-600">Not Run</span>;
                  }
                  return (
                    ...
                    >
                      ...
                      <span className="text-sm font-medium text-slate-800 flex-1 truncate">{sc.name}</span>
                      {family && showFamilies && (
              ...
              <div className="p-3 border-t border-slate-200 shrink-0">
                <button
                  ...
                  onClick={() => { setShowNewForm(true); setSelectedId(null); setFamilyRecordsId(null); }}
                >
                  <Plus className="h-3.5 w-3.5" /> New What-if
                </button>
              </div>
            </>
          ) : (
      ...
      {/* ═══ Right: Content panel — white, thin vertical divider from list ═══ */}
...
function EmptyState({ onNew }: { onNew: () => void }) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-white">
      <FlaskConical className="h-16 w-16 sm:h-20 sm:w-20 mb-5 text-slate-300 opacity-60" strokeWidth={1.25} />
      <p className="text-sm text-slate-600 mb-6">Select a scenario to view or edit it</p>
      <button
        type="button"
        ...
        onClick={onNew}
      >
        <Plus className="h-4 w-4" /> New What-if
      </button>
    </div>
  );
}
...
function NewWhatIfForm({
  ...
  return (
    <div className="flex-1 flex items-center justify-center p-8 bg-white">
      <form onSubmit={handleSubmit} className="w-full max-w-md rounded-lg border border-slate-200 bg-[#FAFAFA]/80 p-6 space-y-5 shadow-sm">
        <h3 className="text-base font-semibold text-slate-900">New What-if</h3>
        <p className="text-xs text-slate-500">Enter a name and optional description, then add the scenario to start editing.</p>
        <div>
          <Label htmlFor="whatif-name" className="text-xs font-medium text-slate-700">Name (required)</Label>
          <Input
            id="whatif-name"
            value={newName}
        ...
        <div>
          <Label htmlFor="whatif-comment" className="text-xs font-medium text-slate-700">Comment (optional)</Label>
          <Textarea
            id="whatif-comment"
            value={newComment}
        ...
        <div className="flex items-center gap-3 pt-1">
          ...
          >
            {isCreating ? (
              ...
              <>
                <Plus className="h-4 w-4" /> Add What-if
              </>
            )}
          </button>
...
function BasecaseView({ model }: { model: Model }) {
  ...
  const tabLabels: Record<string, string> = { general: 'General', labor: 'Labor', equipment: 'Equipment', products: 'Products', operations: 'Operations', ibom: 'Ibom' };
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-[#FAFAFA]/50">
      <div className="h-12 px-6 flex items-center gap-2 border-b border-slate-200 shrink-0">
        <Lock className="h-4 w-4 text-slate-500 shrink-0" />
        <h2 className="text-base font-semibold text-slate-900">Basecase</h2>
        <span className="rounded-full px-2 py-0.5 text-[10px] font-medium bg-[#4CAF50]/20 text-[#2E7D32]">Read-only</span>
      </div>
      <div className="px-6 py-2 text-xs text-slate-500 border-b border-slate-200 shrink-0">
        To edit Basecase data, use the Input screens in the main navigation.
      </div>
      <div className="border-b border-slate-200 shrink-0">
        <div className="flex px-6 gap-0">
          {(['general', 'labor', 'equipment', 'products', 'operations', 'ibom'] as const).map(tab => (
            <button
              key={tab}
            ...
            >
              {tabLabels[tab]}
              {activeTab === tab && <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#20B2AA]" />}
            </button>
          ))}
        </div>
      </div>
...
function ReadOnlyTable({
  ...
  const getAlign = (j: number) => align[j] ?? 'left';
  return (
        ...
        <thead>
          <tr className="bg-slate-100">
            {headers.map((h, j) => (
              <th
                key={h}
                className={`py-3 px-4 font-semibold text-slate-800 border-b border-slate-200 ${
                  getAlign(j) === 'right' ? 'text-right' : getAlign(j) === 'center' ? 'text-center' : 'text-left'
                }`}
              >
                {h}
              </th>
              ...
              {row.map((cell, j) => (
                <td
                  ...
                  className={`py-3 px-4 text-slate-700 ${
                    j === 0 ? 'font-medium text-slate-900' : 'font-mono tabular-nums'
                  } ${
                    getAlign(j) === 'right' ? 'text-right' : getAlign(j) === 'center' ? 'text-center' : 'text-left'
                  }`}
                >
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-x-10 gap-y-4">
      {items.map(i => (
        <div key={i.label} className="flex flex-col gap-0.5">
          ...
          <p className="text-sm font-medium text-slate-900 font-mono">{typeof i.value === 'string' && ['DAY','YEAR','MIN','HOUR'].includes((i.value as string).toUpperCase()) ? (i.value as string).toUpperCase() : i.value}</p>
        </div>
      ))}
    </div>
  );
...
function ReadOnlyGeneralTab({ model }: { model: Model }) {
  ...
  const fmt = (v: string) => (['DAY','YEAR','MIN','HOUR'].includes((v || '').toUpperCase()) ? (v || '-').toUpperCase() : (v || '-'));
  const left = [
    { label: 'Model Title', value: g.model_title || '-' },
    { label: 'MCT Time Unit', value: fmt(g.mct_time_unit) },
    { label: 'Conv2', value: g.conv2 },
    { label: 'Var Labor', value: `${g.var_labor}%` },
  ];
  const middle = [
    { label: 'Author', value: g.author || '-', linkStyle: true },
    { label: 'Production Period', value: fmt(g.prod_period_unit) },
    { label: 'Utilisation Limit', value: `${g.util_limit}%` },
    { label: 'Var Prod', value: `${g.var_prod}%` },
  ];
  const right = [
    { label: 'Ops Time Unit', value: fmt(g.ops_time_unit) },
    { label: 'Conv1', value: g.conv1 },
    { label: 'Var Equip', value: `${g.var_equip}%` },
  ];
  const renderCol = (items: { label: string; value: string | number; linkStyle?: boolean }[]) => (
    <div className="flex flex-col gap-5">
      {items.map(i => (
        <div key={i.label} className="flex flex-col gap-0.5">
          <span className="text-xs font-medium text-slate-500">{i.label}</span>
          <p className={`text-sm font-semibold text-slate-900 font-mono ${i.linkStyle ? 'underline decoration-slate-400 underline-offset-1 cursor-default' : ''}`}>
            {i.value}
          </p>
        </div>
      ))}
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-x-14 gap-y-0">
      {renderCol(left)}
      {renderCol(middle)}
      {renderCol(right)}
    </div>
...
function ReadOnlyLaborTab({ model }: { model: Model }) {
  if (!model.labor.length) return <p className="text-sm text-slate-500">No labor groups.</p>;
  return (
    <ReadOnlyTable
...
function ReadOnlyEquipmentTab({ model }: { model: Model }) {
  if (!model.equipment.length) return <p className="text-sm text-slate-500">No equipment.</p>;
  return (
    <ReadOnlyTable
    ...
    <div className="w-full overflow-hidden">
      <table className="w-full border-collapse text-sm font-sans">
        <thead>
          <tr className="bg-slate-100">
            {headers.map(h => (
              <th key={h} className="text-left py-3 px-4 font-semibold text-slate-800 border-b border-slate-200">{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {model.products.map(p => (
            <tr key={p.id} className="border-b border-slate-200 last:border-b-0 bg-white">
              <td className="py-3 px-4 font-medium text-slate-900">{p.name}</td>
              <td className="py-3 px-4 text-right font-mono text-slate-700 tabular-nums">{p.demand}</td>
              <td className="py-3 px-4 text-right font-mono text-slate-700 tabular-nums">{p.lot_size}</td>
              <td className="py-3 px-4 text-right font-mono text-slate-700 tabular-nums">{p.tbatch_size}</td>
              <td className="py-3 px-4 text-left font-mono text-slate-700">{p.dept_code || '-'}</td>
              {showInclude && (
...
function ReadOnlyOperationsTab({ model }: { model: Model }) {
  if (!model.operations.length) return <p className="text-sm text-slate-500">No operations.</p>;
  ...
  return (
    <ReadOnlyTable
      headers={['Product', 'Op#', 'Op Name', 'Equipment', 'Eq Setup/Lot', 'Eq Run/Pc']}
      rows={model.operations.map(o => [pm[o.product_id] || '-', o.op_number, o.op_name, em[o.equip_id] || '-', o.equip_setup_lot, o.equip_run_piece])}
      align={['left', 'right', 'left', 'left', 'right', 'right']}
    />
  );
}

function ReadOnlyIBOMTab({ model }: { model: Model }) {
  if (!model.ibom.length) return <p className="text-sm text-slate-500">No IBOM entries.</p>;
  const pm = Object.fromEntries(model.products.map(p => [p.id, p.name]));
  return (
    <ReadOnlyTable
      headers={['Parent', 'Component', 'Units/Assy']}
      rows={model.ibom.map(b => [pm[b.parent_product_id] || '-', pm[b.component_product_id] || '-', b.units_per_assy])}
...
function ScenarioView({
  ...
  let statusBadge: React.ReactNode;
  if (isActive) {
    statusBadge = <span className="rounded-full px-2 py-0.5 text-[10px] font-medium bg-amber-100 text-amber-800">Active</span>;
  } else if (scenario.status === 'calculated') {
    statusBadge = <span className="rounded-full px-2 py-0.5 text-[10px] font-medium bg-[#4CAF50]/20 text-[#2E7D32]">Results Current</span>;
  } else if (hasResults) {
    statusBadge = <span className="rounded-full px-2 py-0.5 text-[10px] font-medium border border-amber-300 text-amber-700">Stale</span>;
  } else {
    statusBadge = <span className="rounded-full px-2 py-0.5 text-[10px] font-medium bg-slate-100 text-slate-600">Not Run</span>;
  }
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="h-12 px-6 flex items-center gap-3 border-b border-border shrink-0">
        ...
        {isActive ? (
          ...
          <>
            <Button size="sm" className="bg-[#4CAF50] hover:bg-[#43A047] text-white border-0 h-9 px-4 text-sm font-medium" onClick={onActivate}>
              <Play className="h-3.5 w-3.5 mr-1.5" /> Activate for Editing
            </Button>
            <Button size="sm" variant="outline" className="border-red-400 text-red-600 hover:bg-red-50 hover:text-red-700 h-9 px-4 text-sm font-medium" onClick={onDelete}>
            ...
            return (
              ...
              >
                ...
                {isChangesTab && scenario.changes.length > 0 && (
                  <span className="text-[10px] text-slate-500">({scenario.changes.length})</span>
                )}
                {hasChanges && (
                  <span className="inline-block w-1.5 h-1.5 rounded-full bg-amber-500" />
                )}
                {activeTab === tab.key && <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#20B2AA]" />}
              </button>
            );
          })}
        </div>
      ...
      {/* ── TAB CONTENT — 24px padding, white background ── */}
      <div className="flex-1 overflow-y-auto p-6 bg-white">
        {isActive ? (
          <>
...
function DeltaCell({ basecaseValue, whatIfValue }: { basecaseValue: number | string; whatIfValue: string | number | null }) {
  if (whatIfValue == null) return <td className="py-3 px-4 text-center font-mono text-sm text-slate-400">-</td>;
  const base = Number(basecaseValue);
  const wi = Number(whatIfValue);
  if (isNaN(base) || isNaN(wi) || base === wi) return <td className="py-3 px-4 text-center font-mono text-sm text-slate-400">-</td>;
  const delta = wi - base;
  return (
    ...
    <div className="w-full overflow-hidden">
      <table className="w-full border-collapse text-sm font-sans">
        <thead>
          <tr className="bg-slate-100">
            {headers.map(h => (
              <th key={h} className="text-left py-3 px-4 font-semibold text-slate-800 border-b border-slate-200">
                {h}
              </th>
            ))}
            <th className="text-right py-3 px-4 font-semibold text-slate-800 border-b border-slate-200">Basecase Value</th>
            <th className="text-right py-3 px-4 font-semibold text-slate-800 border-b border-slate-200 min-w-[7rem]">What-if Value</th>
            <th className="text-center py-3 px-4 font-semibold text-slate-800 border-b border-slate-200 w-20">Change</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r, i) => {
            ...
            return (
              <tr key={`${r.entityId}-${r.field}-${i}`} className={`border-b border-slate-200 last:border-b-0 ${wiVal != null && String(wiVal) !== String(r.basecaseValue) ? 'bg-amber-50/50' : ''}`}>
                <td className="py-3 px-4 text-left font-medium text-slate-900 align-middle">{r.entityName}</td>
                {headers.length > 1 && <td className="py-3 px-4 text-left text-slate-600 align-middle">{r.fieldLabel}</td>}
                <td className="py-3 px-4 text-right font-mono font-medium text-slate-700 align-middle tabular-nums">{r.basecaseValue}</td>
...
function EditableGeneralTab({ model, scenario, getWhatIfValue, onBlur }: EditableTabProps) {
  const g = model.general;
  const rows: ParamRow[] = [
    { dataType: 'General', entityId: 'general', entityName: 'General', field: 'conv1', fieldLabel: 'Conv1 (Shifts x Hours)', basecaseValue: g.conv1 },
