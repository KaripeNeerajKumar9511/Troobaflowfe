...
type Props = {
  value: string;
  isEditing: boolean;
  /** When true, name is display-only (e.g. another user holds page edit lock). */
  readOnly?: boolean;
  onRequestEdit: () => void;
...
export function DoubleClickEditableName({
  ...
  return (
    <span
      title={readOnly ? undefined : 'Double-click to edit name'}
      className={cn(
        'font-mono font-medium',
        readOnly
          ? 'cursor-default text-foreground'
          : 'cursor-text underline-offset-2 hover:underline decoration-dotted',
        spanClassName,
      )}
      onDoubleClick={
        readOnly
