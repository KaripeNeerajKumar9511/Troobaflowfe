...
import { adminUserDetail, adminOrgMemberModel, apiJson } from '@/lib/api';
...
export function AdminUserDetail() {
  ...
  const membersBackPath = orgId ? `/TF-admin/organizations/${orgId}/members` : '/TF-admin/users';
  const modelPath = (modelId: string) =>
    orgId
      ? adminOrgMemberModel(orgId, userId!, modelId)
      : `/TF-admin/users/${userId}/models/${modelId}`;
  ...
  useEffect(() => {
    if (data?.user) {
      ...
      setTitleLink(membersBackPath);
    }
    return () => setTitleLink(null);
  }, [data, membersBackPath, setContextTitle, setContextSubtitle, setTitleLink]);
  ...
  return (
    <div className="p-6 max-w-5xl animate-fade-in">
      <div className="mb-6">
        <button
          type="button"
          onClick={() => navigate(membersBackPath)}
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          ...
          {/* Interface level disabled: <Badge variant="outline">Level {user.user_level}</Badge> */}
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {models.map((m) => (
            <Card
              ...
              onClick={() => navigate(modelPath(m.id))}
              role="button"
              tabIndex={0}
              onKeyDown={(e) => e.key === 'Enter' && navigate(modelPath(m.id))}
            >
