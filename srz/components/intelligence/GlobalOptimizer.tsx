...
export function GlobalOptimizer({ model, results }: Props) {
  ...
  const runOptimizer = useCallback(async () => {
    ...
    try {
    // Before metrics
    const beforeResults = results || await fullCalculate(model);
    ...
    const beforeBottleneck = Math.max(0, ...beforeResults.equipment.map(e => e.totalUtil));
    ...
    for (let iter = 1; iter <= maxIter; iter++) {
      ...
      const trialResults = await fullCalculate(trialModel);
      const trialWip = trialResults.products.reduce((s, p) => s + p.wip, 0);
      const trialMaxUtil = Math.max(0, ...trialResults.equipment.map(e => e.totalUtil));
    ...
    const afterResults = await fullCalculate(bestModel);
    ...
    const afterBottleneck = Math.max(0, ...afterResults.equipment.map(e => e.totalUtil));
    ...
    setStep(3);
      ...
      console.error(e);
      toast({ title: 'Optimizer failed', description: e instanceof Error ? e.message : 'Could not run optimization', variant: 'destructive' });
      setStep(1);
    } finally {
      setRunning(false);
    }
