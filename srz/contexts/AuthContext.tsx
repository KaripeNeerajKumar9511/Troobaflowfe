...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const refreshProfile = useCallback(async () => {
    try {
      ...
      // Interface level disabled — no sync from profile.
  ...
  const signIn = async (email: string, password: string) => {
    ...
    if (!res.ok) {
      ...
      return {
        error: new Error(j.error || 'Login failed'),
        accountFrozen: j.code === 'account_frozen',
      };
    }
    const j = await res.json().catch(() => ({} as any));
    const mustChangePassword = Boolean(j?.profile?.must_change_password);
